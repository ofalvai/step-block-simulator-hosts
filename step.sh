...
DENO_VERSION="2.0.4"
DENO_DIR=${TMPDIR}/deno-${DENO_VERSION}
OS_TYPE=$(uname -s)
ARCH_TYPE=$(uname -m)
...
if [[ ! -d "${DENO_DIR}" ]]; then
    ...
    mkdir -p "${DENO_DIR}"
    curl --retry 5 -fsSL "$DENO_URL" -o "${DENO_DIR}"/deno.zip
    unzip "${DENO_DIR}"/deno.zip -d "${DENO_DIR}"
    rm "${DENO_DIR}"/deno.zip

    chmod +x "${DENO_DIR}"/deno
    echo "☑️ Deno is ready at ${DENO_DIR}"
else
    echo "☑️ Using existing Deno binary at ${DENO_DIR}"
fi
